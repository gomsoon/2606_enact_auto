...
This slice does not add literal set or bag syntax, dot-method constructor syntax, custom collection printing, or new Bag-specific algebra. It only makes the existing constructor names useful with ordinary list payloads. Slice 071 later adds binary Bag algebraic operations; Slice 072 later adds list-of-Bag aggregate `UNION`.
