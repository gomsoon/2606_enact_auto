...
- `suppliers(object, attr)` is later added by Slice 085 as a compatibility shorthand for `suppliers(classof(object), attr)`.
...
- Non-class, non-object first arguments shall report `ENACT_ERR_TYPE_EXPECTED_CLASS_OR_OBJECT` after Slice 085.
- Non-atom second arguments shall report `ENACT_ERR_TYPE_EXPECTED_ATOM`.
- Object values are later accepted directly as the first argument by Slice 085.
