...
`reduce` returns the reducer's accumulator value and does not construct a new collection result by itself. Collection-aware `map`, set-operation builtins, dot-method syntax, and custom collection printing remain deferred.
