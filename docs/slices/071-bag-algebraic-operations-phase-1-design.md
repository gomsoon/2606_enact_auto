...
This slice adds binary Bag algebra for `union`, `difference`, and `intersection` only. Bag-aware aggregate `UNION` is deferred in this slice and later added by Slice 072. Dot-method collection syntax remains deferred. Slice 073 later adds kind-aware collection payload display.
