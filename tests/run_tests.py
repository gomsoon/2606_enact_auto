...
def main() -> int:
    ...
    load_bye_path = write_load_case("bye.en", "loaded_before_bye:=10\nloaded_before_bye\nbye\nloaded_before_bye+1\n")
    ...
    native_collection_effective_methods = (
        "'size:'isEmpty:'union:'difference:'intersection:'subset:'equal:'UNION:'member:'insert:'remove:"
        "'add:'collect:'filter:'select:'all:'exists:'locate:'forEachDo:'reduce:nil\n"
    )
    native_collection_effective_methods_without_member = (
        "'size:'isEmpty:'union:'difference:'intersection:'subset:'equal:'UNION:'insert:'remove:"
        "'add:'collect:'filter:'select:'all:'exists:'locate:'forEachDo:'reduce:nil\n"
    )
    ...
    slice_105_boundary_success_cases = [
        ...
        ("member('isEmpty,effectiveMethods(set()))\n", "true\n"),
    ...
    slice_111_boundary_success_cases = [
        ("isNil(nil)\n", "true\n"),
        ("isNil(())\n", "true\n"),
        ("isNil(tl(1:nil))\n", "true\n"),
        ("isNil(filter(x::false,(1,2)))\n", "true\n"),
        ("isNil(1:nil)\n", "false\n"),
        ("isNil((1,2))\n", "false\n"),
        ("isNil(list(nil))\n", "false\n"),
        ("isNil(list(1))\n", "false\n"),
        ...
        ("isNil(bag())\n", "false\n"),
        ("isNil(insert(1,set()))\n", "false\n"),
        ("isList(nil) and isNil(nil)\n", "true\n"),
        ("atom(nil) and isNil(nil)\n", "true\n"),
        ("isList(list(nil)) and not isNil(list(nil))\n", "true\n"),
        ("map(isNil,(nil,(),1:nil,list(nil),1,set()))\n", "true:true:false:false:false:false:nil\n"),
        ("filter(isNil,(nil,(),1:nil,list(nil),1))\n", "(nil):(nil):nil\n"),
        ("all(isNil,(nil,()))\n", "true\n"),
        ("not isNil(1:nil)\n", "true\n"),
        ("callableParams(isNil)\n", "'value:nil\n"),
        ("isNil:=x::false\nisNil(nil)\n", "<function>\nfalse\n"),
    ]

    slice_111_robustness_failure_cases = [
        ("isNil()\n", "ENACT_ERR_ARITY_MISMATCH"),
        ("isNil(nil,1/0)\n", "ENACT_ERR_ARITY_MISMATCH"),
        ("isNil(missing)\n", "ENACT_ERR_NAME_UNBOUND"),
        ("isNil(1/0)\n", "ENACT_ERR_DIVIDE_BY_ZERO"),
        ...
        ("isNil(nil)(1)\n", "ENACT_ERR_TYPE_EXPECTED_FUNCTION"),
        ("map(isNil,1)\n", "ENACT_ERR_TYPE_EXPECTED_LIST"),
        ("reduce(isNil,0,list(nil))\n", "ENACT_ERR_ARITY_MISMATCH"),
        ("isNil:=1\nisNil(nil)\n", "ENACT_ERR_TYPE_EXPECTED_FUNCTION"),
    ]

    slice_112_boundary_success_cases = [
        ("isEmpty(nil)\n", "true\n"),
        ("isEmpty(())\n", "true\n"),
        ("isEmpty(tl(1:nil))\n", "true\n"),
        ("isEmpty(filter(x::false,(1,2)))\n", "true\n"),
        ("isEmpty(set())\n", "true\n"),
        ("isEmpty(bag())\n", "true\n"),
        ("isEmpty(filter(x::false,set((1,2))))\n", "true\n"),
        ("isEmpty(filter(x::false,bag((1,2))))\n", "true\n"),
        ("class MySet < Set\nisEmpty(new MySet)\n", "<class MySet>\ntrue\n"),
        ("class MyBag < Bag\nisEmpty(new MyBag)\n", "<class MyBag>\ntrue\n"),
        ("isEmpty(1:nil)\n", "false\n"),
        ("isEmpty((1,2))\n", "false\n"),
        ("isEmpty(list(nil))\n", "false\n"),
        ("isEmpty(insert(1,set()))\n", "false\n"),
        ("isEmpty(insert(1,bag()))\n", "false\n"),
        ("isEmpty(1)\n", "false\n"),
        ("isEmpty(true)\n", "false\n"),
        ('isEmpty("x")\n', "false\n"),
        ("isEmpty('x)\n", "false\n"),
        ...
        ("isEmpty(hd)\n", "false\n"),
        ("isEmpty(append(nil))\n", "false\n"),
        ("isEmpty(set() with tag:=true)\n", "true\n"),
        ("isNil(nil) and isEmpty(nil)\n", "true\n"),
        ("not isNil(set()) and isEmpty(set())\n", "true\n"),
        ("not isNil(bag()) and isEmpty(bag())\n", "true\n"),
        ("isList(list(nil)) and not isEmpty(list(nil))\n", "true\n"),
        ("isCollection(set()) and isEmpty(set())\n", "true\n"),
        (
            "map(isEmpty,(nil,(),1:nil,set(),insert(1,set()),bag(),insert(1,bag()),1))\n",
            "true:true:false:true:false:true:false:false:nil\n",
        ),
        (
            "filter(isEmpty,(nil,(),1:nil,set(),insert(1,set()),bag(),insert(1,bag()),1))\n",
            "(nil):(nil):set():bag():nil\n",
        ),
        ("all(isEmpty,(nil,(),set(),bag()))\n", "true\n"),
        ("not isEmpty(1:nil)\n", "true\n"),
        ("callableParams(isEmpty)\n", "'value:nil\n"),
        ("isEmpty:=x::false\nisEmpty(nil)\n", "<function>\nfalse\n"),
    ]

    slice_112_robustness_failure_cases = [
        ("isEmpty()\n", "ENACT_ERR_ARITY_MISMATCH"),
        ("isEmpty(nil,1/0)\n", "ENACT_ERR_ARITY_MISMATCH"),
        ("isEmpty(missing)\n", "ENACT_ERR_NAME_UNBOUND"),
        ("isEmpty(1/0)\n", "ENACT_ERR_DIVIDE_BY_ZERO"),
        ...
        ("isEmpty(nil)(1)\n", "ENACT_ERR_TYPE_EXPECTED_FUNCTION"),
        ("map(isEmpty,1)\n", "ENACT_ERR_TYPE_EXPECTED_LIST"),
        ("reduce(isEmpty,0,list(nil))\n", "ENACT_ERR_ARITY_MISMATCH"),
        ("isEmpty:=1\nisEmpty(nil)\n", "ENACT_ERR_TYPE_EXPECTED_FUNCTION"),
    ]

    slice_113_boundary_success_cases = [
        ("set().isEmpty()\n", "true\n"),
        ("bag().isEmpty()\n", "true\n"),
        ("set((1,2)).isEmpty()\n", "false\n"),
        ("bag((1,1)).isEmpty()\n", "false\n"),
        ("set().insert(1).remove(1).isEmpty()\n", "true\n"),
        ("bag((1,1)).remove(1).remove(1).isEmpty()\n", "true\n"),
        ("f:=set().isEmpty\nf()\n", "<function>\ntrue\n"),
        ("f:=set(list(1)).isEmpty\nf()\n", "<function>\nfalse\n"),
        (
            "map(x::x.isEmpty(),(set(),bag(),set(list(1)),bag(list(1))))\n",
            "true:true:false:false:nil\n",
        ),
        (
            "filter(x::x.isEmpty(),(set(),bag(),set(list(1)),bag(list(1))))\n",
            "set():bag():nil\n",
        ),
        ("all(x::x.isEmpty(),(set(),bag()))\n", "true\n"),
        ("class DotSet < Set\n(new DotSet).isEmpty()\n", "<class DotSet>\ntrue\n"),
        ("class DotBag < Bag\n(new DotBag).insert(1).isEmpty()\n", "<class DotBag>\nfalse\n"),
        ("isCallable(set().isEmpty)\n", "true\n"),
        ("callableArity(set().isEmpty)\n", "0\n"),
        ("callableMinArity(set().isEmpty)\n", "0\n"),
        ("callableArityRange(set().isEmpty)\n", "0:0:nil\n"),
        ("callableParams(set().isEmpty)\n", "nil\n"),
        ("methodArity(set(),'isEmpty)\n", "0\n"),
        ("methodParams(set(),'isEmpty)\n", "nil\n"),
        ("hasMethod(set(),'isEmpty)\n", "true\n"),
        ("member('isEmpty,effectiveMethods(set()))\n", "true\n"),
        ("size(effectiveMethods(set()))\n", "20\n"),
        ("hd(tl(effectiveMethods(set())))\n", "'isEmpty\n"),
        ("isEmpty:=x::false\nset().isEmpty()\n", "<function>\ntrue\n"),
        ("Set.isEmpty():=false\nset().isEmpty()\n", "<function>\nfalse\n"),
        ("f():=42\ns:=set() with isEmpty:=f\ns.isEmpty()\n", "<function>\nset()\n42\n"),
        ("methods(Set)\n", "nil\n"),
    ]

    slice_113_robustness_failure_cases = [
        ("set().isEmpty(1/0)\n", "ENACT_ERR_ARITY_MISMATCH"),
        ("set().isEmpty(1)\n", "ENACT_ERR_ARITY_MISMATCH"),
        ("f:=set().isEmpty\nf(1/0)\n", "ENACT_ERR_ARITY_MISMATCH"),
        ("set().isEmpty+1\n", "ENACT_ERR_TYPE_EXPECTED_INT"),
        ...
        ("(new Object).isEmpty()\n", "ENACT_ERR_ATTRIBUTE_UNBOUND"),
        ("(1).isEmpty()\n", "ENACT_ERR_TYPE_EXPECTED_OBJECT"),
        ("s:=set() with isEmpty:=1\ns.isEmpty()\n", "ENACT_ERR_TYPE_EXPECTED_FUNCTION"),
        ("Set.isEmpty():=1/0\nset().isEmpty()\n", "ENACT_ERR_DIVIDE_BY_ZERO"),
        ("hd(methodParams(set(),'isEmpty))\n", "ENACT_ERR_LIST_EMPTY"),
        ("filter(x::x.isEmpty(),(set(),new Object))\n", "ENACT_ERR_ATTRIBUTE_UNBOUND"),
    ]

    slice_114_boundary_success_cases = [
        ("isInt(time())\n", "true\n"),
        ("time()>=0\n", "true\n"),
        ("time\n", "<function>\n"),
        ("isCallable(time)\n", "true\n"),
        ("callableArity(time)\n", "0\n"),
        ("callableMinArity(time)\n", "0\n"),
        ("callableArityRange(time)\n", "0:0:nil\n"),
        ("callableParams(time)\n", "nil\n"),
        ("hd(callableArityRange(time))==0\n", "true\n"),
        ("apply0(f):=f()\nisInt(apply0(time))\n", "<function>\ntrue\n"),
        ("f:=time\nisInt(f())\n", "<function>\ntrue\n"),
        ("list(time)\n", "<function>:nil\n"),
        ("map(f::isInt(f()),list(time))\n", "true:nil\n"),
        ("isObject(time())\n", "false\n"),
        ("atom(time())\n", "true\n"),
        ("not isNil(time())\n", "true\n"),
        ("time:=()::123\ntime()\n", "<function>\n123\n"),
    ...
    slice_114_robustness_failure_cases = [
        ("time(1/0)\n", "ENACT_ERR_ARITY_MISMATCH"),
        ("time(1)\n", "ENACT_ERR_ARITY_MISMATCH"),
        ("time()+true\n", "ENACT_ERR_TYPE_EXPECTED_INT"),
        ("not time()\n", "ENACT_ERR_TYPE_EXPECTED_BOOL"),
        ...
        ("filter(time,(1,2))\n", "ENACT_ERR_ARITY_MISMATCH"),
        ("reduce(time,0,(1,2))\n", "ENACT_ERR_ARITY_MISMATCH"),
        ("callableParams(time)(1)\n", "ENACT_ERR_TYPE_EXPECTED_FUNCTION"),
        ("time:=1\ntime()\n", "ENACT_ERR_TYPE_EXPECTED_FUNCTION"),
    ]

    slice_115_boundary_success_cases = [
        ("bye\n", ""),
        ("bye.\n", ""),
        ("   % before bye\nbye % trailing comment\n", ""),
        ("x:=1\nbye\nx+1\n", "1\n"),
        ("x:=1.\nbye.\nx+1.\n", "1\n"),
        ("x:=1. bye. x+1.", "1\n"),
        (f'load "{load_bye_path}"\n999\n', "10\n10\n"),
        ("byebye:=3\nbyebye\n", "3\n3\n"),
        ("bye_value:=4\nbye_value\n", "4\n4\n"),
    ]

    slice_115_robustness_failure_cases = [
        ("bye", "ENACT_ERR_PARSE_MISSING_DOT"),
        ("bye 1\n", "ENACT_ERR_PARSE_UNEXPECTED_TOKEN"),
        ("bye()\n", "ENACT_ERR_PARSE_UNEXPECTED_TOKEN"),
        ("bye:=1\n", "ENACT_ERR_PARSE_UNEXPECTED_TOKEN"),
        ("bye.x\n", "ENACT_ERR_PARSE_UNEXPECTED_TOKEN"),
    ...
    slice_116_boundary_success_cases = [
        ...
        ("isCallable(ask)\n", "true\n"),
        ("callableArity(ask)\n", "0\n"),
        ("callableMinArity(ask)\n", "0\n"),
        ("callableArityRange(ask)\n", "0:0:nil\n"),
        ("callableParams(ask)\n", "nil\n"),
        ("list(ask)\n", "<function>:nil\n"),
        ("ask:=()::\"local\"\nask()\n", "<function>\n\"local\"\n"),
    ]

    slice_116_robustness_failure_cases = [
        ("ask()\n", "ENACT_ERR_INPUT_UNAVAILABLE"),
        ("ask(1/0)\n", "ENACT_ERR_ARITY_MISMATCH"),
        ("ask(1)\n", "ENACT_ERR_ARITY_MISMATCH"),
        ("map(ask,(1,2))\n", "ENACT_ERR_ARITY_MISMATCH"),
        ("filter(ask,(1,2))\n", "ENACT_ERR_ARITY_MISMATCH"),
    ...
    slice_118_boundary_success_cases = [
        ("isInt(cells())\n", "true\n"),
        ("cells()>=0\n", "true\n"),
        ("cells\n", "<function>\n"),
        ("isCallable(cells)\n", "true\n"),
        ("callableArity(cells)\n", "0\n"),
        ("callableMinArity(cells)\n", "0\n"),
        ("callableArityRange(cells)\n", "0:0:nil\n"),
        ("callableParams(cells)\n", "nil\n"),
        ("hd(callableArityRange(cells))==0\n", "true\n"),
        ("apply0(f):=f()\nisInt(apply0(cells))\n", "<function>\ntrue\n"),
        ("f:=cells\nisInt(f())\n", "<function>\ntrue\n"),
        ("list(cells)\n", "<function>:nil\n"),
        ("map(f::isInt(f()),list(cells))\n", "true:nil\n"),
        ("isObject(cells())\n", "false\n"),
        ("atom(cells())\n", "true\n"),
        ("not isNil(cells())\n", "true\n"),
        ("before:=cells(); holder:=list(1); after:=cells(); after>before\n", "true\n"),
        ("cells:=()::123\ncells()\n", "<function>\n123\n"),
        ("cells:=()::123\ncallableParams(cells)\n", "<function>\nnil\n"),
    ]

    slice_118_robustness_failure_cases = [
        ("cells(1/0)\n", "ENACT_ERR_ARITY_MISMATCH"),
        ("cells(1)\n", "ENACT_ERR_ARITY_MISMATCH"),
        ("cells()+true\n", "ENACT_ERR_TYPE_EXPECTED_INT"),
        ("not cells()\n", "ENACT_ERR_TYPE_EXPECTED_BOOL"),
        ...
        ("filter(cells,(1,2))\n", "ENACT_ERR_ARITY_MISMATCH"),
        ("reduce(cells,0,(1,2))\n", "ENACT_ERR_ARITY_MISMATCH"),
        ("callableParams(cells)(1)\n", "ENACT_ERR_TYPE_EXPECTED_FUNCTION"),
        ("cells:=1\ncells()\n", "ENACT_ERR_TYPE_EXPECTED_FUNCTION"),
    ]

    slice_119_boundary_success_cases = [
        ("isInt(maxcells())\n", "true\n"),
        ("maxcells()>=0\n", "true\n"),
        ("maxcells()>=cells()\n", "true\n"),
        ("maxcells\n", "<function>\n"),
        ("isCallable(maxcells)\n", "true\n"),
        ("callableArity(maxcells)\n", "0\n"),
        ("callableMinArity(maxcells)\n", "0\n"),
        ("callableArityRange(maxcells)\n", "0:0:nil\n"),
        ("callableParams(maxcells)\n", "nil\n"),
        ("hd(callableArityRange(maxcells))==0\n", "true\n"),
        ("apply0(f):=f()\nisInt(apply0(maxcells))\n", "<function>\ntrue\n"),
        ("f:=maxcells\nisInt(f())\n", "<function>\ntrue\n"),
        ("list(maxcells)\n", "<function>:nil\n"),
        ("map(f::isInt(f()),list(maxcells))\n", "true:nil\n"),
        ("isObject(maxcells())\n", "false\n"),
        ("atom(maxcells())\n", "true\n"),
        ("not isNil(maxcells())\n", "true\n"),
        ("before:=maxcells(); holder:=list(1); after:=maxcells(); after>before\n", "true\n"),
        ("before:=maxcells(); (1 where tmp:=list(1)); after:=maxcells(); after>before\n", "true\n"),
        ("maxcells:=()::123\nmaxcells()\n", "<function>\n123\n"),
        ("maxcells:=()::123\ncallableParams(maxcells)\n", "<function>\nnil\n"),
    ]

    slice_119_robustness_failure_cases = [
        ("maxcells(1/0)\n", "ENACT_ERR_ARITY_MISMATCH"),
        ("maxcells(1)\n", "ENACT_ERR_ARITY_MISMATCH"),
        ("maxcells()+true\n", "ENACT_ERR_TYPE_EXPECTED_INT"),
        ("not maxcells()\n", "ENACT_ERR_TYPE_EXPECTED_BOOL"),
        ...
        ("map(maxcells,(1,2))\n", "ENACT_ERR_ARITY_MISMATCH"),
        ("filter(maxcells,(1,2))\n", "ENACT_ERR_ARITY_MISMATCH"),
        ("reduce(maxcells,0,(1,2))\n", "ENACT_ERR_ARITY_MISMATCH"),
        ("callableParams(maxcells)(1)\n", "ENACT_ERR_TYPE_EXPECTED_FUNCTION"),
        ("maxcells:=1\nmaxcells()\n", "ENACT_ERR_TYPE_EXPECTED_FUNCTION"),
    ]

    slice_121_boundary_success_cases = [
        ("fact(n):=n==0 then 1 else n*fact(n-1); fact(5)\n", "120\n"),
        ("factorial(n):=n==0 then 1 else n*factorial(n-1); factorial(6)\n", "720\n"),
        ("nfib(n):=n<2 then 1 else nfib(n-1)+nfib(n-2)+1; nfib(5)\n", "15\n"),
        ("reverse(x):=x==nil then nil else append(reverse(tl x),(hd x):()); reverse((1,2,3))\n", "3:2:1:nil\n"),
        ("twice(f,x):=f(f(x)); inc(x):=x+1; twice(inc,3)\n", "5\n"),
        ("twice(f):=x::f(f(x)); inc(x):=x+1; add2:=twice(inc); add2(5)\n", "7\n"),
        ("twice f x:=f(f(x)); inc x:=x+1; twice inc 4\n", "6\n"),
        ("twice(f):=x::f(f(x)); double(x):=x*2; map(twice(double),(1,2,3))\n", "4:8:12:nil\n"),
        ("compose(f,g,x):=f(g(x)); twice(f,x):=compose(f,f,x); inc(x):=x+1; twice(inc,10)\n", "12\n"),
        ("fact(n):=n==0 then 1 else n*fact(n-1)\nfact(5)\n", "<function>\n120\n"),
        ("reverse(x):=x==nil then nil else append(reverse(tl x),(hd x):())\nreverse((1,2,3,4))\n", "<function>\n4:3:2:1:nil\n"),
        ("twice(f):=x::f(f(x))\ninc(x):=x+1\ntwice(inc)(8)\n", "<function>\n<function>\n10\n"),
    ]

    slice_121_robustness_failure_cases = []

    slice_122_boundary_success_cases = [
        ("class Tree < Object\nclass Leaf < Tree\nclassof(new Leaf)==Leaf\n", "<class Tree>\n<class Leaf>\ntrue\n"),
        ("class Tree < Object\nTree.payload():=self.value\nclass Leaf < Tree\n(new Leaf with value:=7).payload()\n", "<class Tree>\n<function>\n<class Leaf>\n7\n"),
        ("class Tree < Object\nTree.payload():=self.value\nclass Leaf < Tree\nmap(x::x.payload(),(new Leaf with value:=1,new Leaf with value:=2))\n", "<class Tree>\n<function>\n<class Leaf>\n1:2:nil\n"),
        ("class Tree < Object\nTree.inc():=self.value:=self.value+1\nnode:=new Tree with value:=1\nnode.inc()\nnode.inc()\nnode.value\n", "<class Tree>\n<function>\n<object Tree>\n2\n3\n3\n"),
        ("class Tree < Object\nTree.parentValue():=self.parent.value\nparent:=new Tree with value:=1\nchild:=new Tree with parent:=parent\nchild.parentValue()\nparent.value:=2\nchild.parentValue()\n", "<class Tree>\n<function>\n<object Tree>\n<object Tree>\n1\n2\n2\n"),
        ("class Tree < Object\nTree.sum():=isNil(self.parent) then self.value else self.value+self.parent.value\nroot:=new Tree with value:=10 with parent:=nil\nleaf:=new Tree with value:=3 with parent:=root\nleaf.sum()\nroot.value:=20\nleaf.sum()\n", "<class Tree>\n<function>\n<object Tree>\n<object Tree>\n13\n20\n23\n"),
        ("class Tree < Object\nTree.kind():='tree\nclass Leaf < Tree\nLeaf.kind():='leaf\n(new Leaf).kind()\n", "<class Tree>\n<function>\n<class Leaf>\n<function>\n'leaf\n"),
        ("class Tree < Object\nTree.value():=1\nnode:=new Tree with value:=()::2\nnode.value()\n", "<class Tree>\n<function>\n<object Tree>\n2\n"),
        ("class Tree < Object\nTree.value():=1\nclass Leaf < Tree\nclasses(Leaf)\n", "<class Tree>\n<function>\n<class Leaf>\n<class Leaf>:<class Tree>:<class Object>:nil\n"),
        ("class Left < Object\nLeft.value():=1\nclass Right < Object\nRight.value():=2\nclass Leaf < (Left,Right)\n(new Leaf).value()\n", "<class Left>\n<function>\n<class Right>\n<function>\n<class Leaf>\n1\n"),
        ("class Left < Object\nLeft.value():=1\nclass Right < Object\nRight.value():=2\nclass Leaf < (Left,Right)\nbadAttrs(Leaf)\n", "<class Left>\n<function>\n<class Right>\n<function>\n<class Leaf>\n'value:nil\n"),
        ("class Left < Object\nLeft.value():=1\nclass Right < Object\nRight.value():=2\nclass Leaf < (Left,Right)\nsuppliers(Leaf,'value)\n", "<class Left>\n<function>\n<class Right>\n<function>\n<class Leaf>\n<class Left>:<class Right>:nil\n"),
        ("class Left < Object\nLeft.value():=1\nclass Right < Object\nRight.value():=2\nclass Leaf < (Left,Right)\nLeaf.value():=3\nbadAttrs(Leaf)\n(new Leaf).value()\n", "<class Left>\n<function>\n<class Right>\n<function>\n<class Leaf>\n<function>\nnil\n3\n"),
        ("class X < Object\nclass Y < Object\nclass A < (X,Y)\nclass B < (Y,X)\nclass C < (A,B)\nOK(C)\n", "<class X>\n<class Y>\n<class A>\n<class B>\n<class C>\nfalse\n"),
        ("class X < Object\nclass Y < Object\nclass A < (X,Y)\nclass B < (Y,X)\nclass C < (A,B)\nOK(C) then 1 else 2\n", "<class X>\n<class Y>\n<class A>\n<class B>\n<class C>\n2\n"),
    ]

    slice_122_robustness_failure_cases = []

    slice_123_boundary_success_cases = [
        ("set()\nbag()\n", "set()\nbag()\n"),
        ("s:=set((1,2,2))\nb:=bag((1,2,2))\nsize(s)\nsize(b)\n", "set(1:2:nil)\nbag(1:2:2:nil)\n2\n3\n"),
        ("member(2,set((1,2)))\nmember(3,set((1,2)))\n", "true\nfalse\n"),
        ("set((1,2)).insert(3).remove(1)\n", "set(3:2:nil)\n"),
        ("bag((1,1,2)).remove(1)\n", "bag(1:2:nil)\n"),
        ("collect(x::x+1,set((1,2)))\n", "set(2:3:nil)\n"),
        ("bag((1,1)).collect(x::x+1)\n", "bag(2:2:nil)\n"),
        ("filter(x::x>1,set((1,2,3)))\n", "set(2:3:nil)\n"),
        ("select(x::x!=2,bag((1,2,2,3)))\n", "bag(1:3:nil)\n"),
        ("reduce((acc,x)::acc+x,0,set((1,2,3)))\n", "6\n"),
        ("union(set((1,2)),set((2,3)))\n", "set(1:2:3:nil)\n"),
        ("difference(set((1,2,3)),set(list(2)))\n", "set(1:3:nil)\n"),
        ("intersection(set((1,2,3)),set((2,4)))\n", "set(2:nil)\n"),
        ("equal(set((1,2)),set((2,1)))\nsubset(set((1,2)),set((1,2,3)))\n", "true\ntrue\n"),
        ("UNION((set((1,2)),set((2,3))))\n", "set(1:2:3:nil)\n"),
        ("set((set(list(1)),set(list(2)))).UNION()\n", "set(1:2:nil)\n"),
        ("bag((bag((1,1)),bag((1,2)))).UNION()\n", "bag(1:1:2:nil)\n"),
        ("set((1,2)).union(set((2,3))).difference(set(list(1))).intersection(set((2,4)))\n", "set(2:nil)\n"),
        ("map(set((1,2)).member,(1,3))\n", "true:false:nil\n"),
        ("class MySet < Set\nclassof((new MySet).insert(1).collect(x::x+1))==MySet\n", "<class MySet>\ntrue\n"),
        ("badAttrs(Set)\nhasMethod(set(),'collect)\n", "nil\ntrue\n"),
    ]

    slice_123_robustness_failure_cases = []
    ...
    print(f"slice 111 boundary regression checks: {len(slice_111_boundary_success_cases)}")
    print(f"slice 111 robustness regression checks: {len(slice_111_robustness_failure_cases)}")
    print(f"slice 112 boundary regression checks: {len(slice_112_boundary_success_cases)}")
    print(f"slice 112 robustness regression checks: {len(slice_112_robustness_failure_cases)}")
    print(f"slice 113 boundary regression checks: {len(slice_113_boundary_success_cases)}")
    print(f"slice 113 robustness regression checks: {len(slice_113_robustness_failure_cases)}")
    print(f"slice 114 boundary regression checks: {len(slice_114_boundary_success_cases)}")
    print(f"slice 114 robustness regression checks: {len(slice_114_robustness_failure_cases)}")
    print(f"slice 115 boundary regression checks: {len(slice_115_boundary_success_cases) + len(slice_115_boundary_tty_cases)}")
    print(f"slice 115 robustness regression checks: {len(slice_115_robustness_failure_cases)}")
    print(f"slice 116 boundary regression checks: {len(slice_116_boundary_success_cases) + len(slice_116_boundary_tty_cases)}")
    print(f"slice 116 robustness regression checks: {len(slice_116_robustness_failure_cases)}")
    print(f"slice 118 boundary regression checks: {len(slice_118_boundary_success_cases)}")
    print(f"slice 118 robustness regression checks: {len(slice_118_robustness_failure_cases)}")
    print(f"slice 119 boundary regression checks: {len(slice_119_boundary_success_cases)}")
    print(f"slice 119 robustness regression checks: {len(slice_119_robustness_failure_cases)}")
    print(f"slice 121 boundary regression checks: {len(slice_121_boundary_success_cases)}")
    print(f"slice 121 robustness regression checks: {len(slice_121_robustness_failure_cases)}")
    print(f"slice 122 boundary regression checks: {len(slice_122_boundary_success_cases)}")
